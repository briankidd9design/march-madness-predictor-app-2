...
const displayData = async () => {
  ...
  let dataDisplay = payload
    .map((data) => {
      return `
            <p>Team Name: ${data.team_name} National Rank: ${data.nat_rank} Schedule Difficulty: ${data.sched_dfclty} Defensive Rating: ${data.def_rating} Offensive Rating: ${data.offsv_rating}</p>
        `;
    })
    .join("");
...
//   let dataDisplay = payload.map((data) => {
//     return `
//             <p>Team Name: ${data.team_name}
//         `;
//   });
//   dataDiv.innerHTML = dataDisplay;
// };
